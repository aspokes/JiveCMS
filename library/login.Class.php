...
//die($uname.'|'.$pwd);
	class Login{
		...
		//die($passwrd);
			$query = "SELECT USR_ID,USR_SURNAME,USR_OTHERNAME,USR_STATUS,USR_ACCESS_LEVEL,USR_CHUR_CODE,USR_BRAN_CODE,USR_LOGIN_STATUS,USR_LASTLOGIN_DATE FROM webmin_users WHERE USR_STATUS='1' AND USR_USERNAME=".$this->connect->Param('a')." AND USR_PASSWORD=".$this->connect->Param('b')."";
			...
			if($stmt){
				if($stmt->RecordCount() > 0){
					...
					$companycode = $arr->USR_CHUR_CODE;
					$branchcode = $arr->USR_BRAN_CODE;
					$useraccesslevel = $arr->USR_ACCESS_LEVEL;
					$this->storeAuth($userid,USER_LOGIN_VAR,$companycode,$branchcode,$infullname,$useraccesslevel);
					$this->setLog("1");
					...
					$query = "INSERT INTO webmin_eventlog (EVL_EVTCODE,EVL_USERID,EVL_FULLNAME,EVL_ACTIVITIES,EVL_IP,EVL_SESSION_ID,EVL_BROWSER,EVL_RAW) VALUES (".$this->connect->Param('a').",".$this->connect->Param('b').",".$this->connect->Param('c').",".$this->connect->Param('d').",".$this->connect->Param('e').",".$this->connect->Param('f').",".$this->connect->Param('g').",".$this->connect->Param('h').")";
	...
	public function storeAuth($userid,$login,$companycode,$branchcode,$infullname,$useraccesslevel){
		...
		$this->connect->Execute("UPDATE webmin_users SET USR_LOGIN_STATUS = '1',USR_LASTLOGIN_DATE = ".$this->connect->Param('a').", USR_LOGIN_IP = ".$this->connect->Param('b')." WHERE USR_ID = ".$this->connect->Param('e'),array(date("Y-m-d H:i:s"),$this->remoteip,$userid));
	}//end
	...
	public function logout($msg="out"){
		...
		if($msg =="out"){
			...
			$this->connect->Execute("UPDATE webmin_users SET USR_LOGIN_STATUS = '0',USR_LASTLOGIN_DATE = ".$this->connect->Param('a')." WHERE USR_ID = ".$this->connect->Param('e'),array(date("Y-m-d H:i:s"),$userid));
			$this->setLog("0");
		}
	...
	public function confirmAuth(){
		...
		if(md5($this->hashkey . $login .$this->remoteip.$this->sessionid.$this->useragent) != $hashkey){
			...
			//UPDATE SESSION
			$userid=$this->session->get("actorid");
			$this->connect->Execute("UPDATE webmin_users SET USR_LASTLOGIN_DATE = ".$this->connect->Param('a')." WHERE USR_ID = ".$this->connect->Param('e'),array(date("Y-m-d H:i:s"),$userid));
		}
	...
	private function setLog($act){
        ...
        $stmt = $this->connect->Execute($query,array($type,$userid,$infullname,$activity,$this->remoteip,$toinsetsession,$this->useragent,$raw));
          print $this->connect->ErrorMsg();
       }
