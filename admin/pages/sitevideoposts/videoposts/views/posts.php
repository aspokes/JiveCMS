<div class="container-fluid">
  ...
  <?php if(empty($keys)){?>
    <button type="submit" onclick="document.getElementById('viewpage').value='saveposts';document.getElementById('view').value='';document.myform.submit;" class="btn btn-success square" title="Save">
      <i class="fa fa-save"></i> Save Video
    </button>
  <?php }else{?>
  ...
  <div class="card mb-3 square">
    <div class="card-header">
      <i class="fa fa-list"></i> Add Video
    ...
    <div class="card-body">
        ...
        <div class="row">
        <div class="form-group col-sm-12">
            <label for="postbody">Video URL</label>
            <input class="form-control" type="text" name="postvideourl" id="postvideourl" placeholder="video url" value="<?php echo(($postvideourl))?$postvideourl:''; ?>" />
          </div>
        </div>

        <div class="row">
          <div class="form-group col-sm-6">
            <label for="postvideoid"> Video ID </label>
            <input class="form-control" type="text" name="postvideoid" id="postvideoid" placeholder="youtube video id" value="<?php echo(($postvideoid))?$postvideoid:''; ?>" />
          </div>
          <div class="form-group col-sm-6">
            ...
            <select name="postcategory" id="postcategory" class="form-control">
              ...
              <option <?php echo(($cat->VIC_ID == $postcategory))?'selected':''; ?> value="<?php echo $cat->VIC_ID; ?>"><?php echo $cat->VIC_NAME; ?></option>
              <?php } ?>
            </select>
          </div>
        </div>
